...
ANATOMY_QUESTION = (
    ...
    "SUB TOPIC DESCRIPTION (CRITICAL CONTEXT):"
    ...
    "--------------------------------------------------"
    "SEMANTIC PRIORITY (CRITICAL - FOLLOW THIS ORDER):"
    "1. SUB_TOPIC defines WHAT must be asked"
    "2. TOPIC defines the anatomical domain"
    "3. DIVERSITY_MODE defines HOW the question is asked"
    "--------------------------------------------------"

    "CORE REQUIREMENTS:"
    "- The question MUST be SHORT and DIRECT"
    "- The question MUST test ONE single concept"
    "- DO NOT use clinical cases"
    "- DO NOT add unnecessary context"
    "- The question MUST NOT be generic"

    "SUBTOPIC ENFORCEMENT (VERY STRICT):"
    "- The question MUST require knowledge specific to {SUB_TOPIC}"
    "- It is FORBIDDEN to generate a question that can be answered without this subtopic"
    "- The content MUST explicitly reflect: {SUBTOPIC_DESCRIPTION}"
    "- If the subtopic is removed and the question still works, the question is INVALID"

    "TOPIC ISOLATION (CRITICAL):"
    "- The question MUST belong strictly to {TOPIC}"
    "- Mixing topics (e.g., Neuro vs Splanchnology vs Locomotor) is STRICTLY FORBIDDEN"
    "- If any structure belongs to another topic → REGENERATE internally before output"

    "DOMAIN ENFORCEMENT:"
    "- Use ONLY structures that belong to the subtopic"
    "- DO NOT introduce unrelated anatomical systems"
    "- All answer options MUST belong to the SAME category (all nerves, all arteries, etc.)"

    "DIVERSITY CONTROL (IMPORTANT):"
    "- Apply {DIVERSITY_MODE} ONLY as a variation of the question format"
    "- DIVERSITY_MODE must NOT change the anatomical content"
    "- Content always comes from SUB_TOPIC, never from diversity mode"

    "ANTI-GENERIC RULE (CRITICAL):"
    "- Avoid generic anatomy questions (e.g., 'main artery', 'main muscle')"
    "- The question MUST involve a specific anatomical detail of the subtopic"

    "ANSWER CONTROL (STRICT - FOLLOW EXACTLY):"
    "STEP 1: Determine the correct answer CONTENT"
    "STEP 2: Validate that ONLY this answer is correct (no ambiguity)"
    "STEP 3: Create 3 alternatives that are plausible but clearly incorrect"
    "STEP 4: Ensure each incorrect option has a specific anatomical error"
    "STEP 5: Place the correct answer ONLY in option {CORRECT_LETTER}"
    "STEP 6: Ensure the correct content does NOT appear in any other option"
    "STEP 7: Set 'correct_answer' = {CORRECT_LETTER}"

    "UNIQUE CORRECT ANSWER (MANDATORY):"
    "- There MUST be exactly ONE correct answer"
    "- No option can be partially correct"
    "- No option can be context-dependent"
    "- If more than one option could be correct → FIX before output"

    "FINAL SEMANTIC VALIDATION (MANDATORY BEFORE OUTPUT):"
    "- Does the question strictly belong to {TOPIC}? If not → REGENERATE"
    "- Does it strictly test {SUB_TOPIC}? If not → REGENERATE"
    "- Does it reflect {SUBTOPIC_DESCRIPTION}? If not → REGENERATE"
    "- Is diversity mode secondary? If not → REGENERATE"
    "- Is there ONLY ONE correct answer? If not → REGENERATE"
